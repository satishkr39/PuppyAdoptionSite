...
qualifier, that is, including the database name and owners name as separate
...
"database" and "owners"  components in order to correctly query the SQL
...
reflection, would be reflected using "dbo" as the owners and "MyDataBase"
as the database name.

To control how the schema name is broken into database / owners,
specify brackets (which in SQL Server are quoting characters) in the name.
Below, the "owners" will be considered as ``MyDataBase.dbo`` and the
...
To individually specify both database and owners name with special characters
...
.. versionchanged:: 1.2 the SQL Server dialect now treats brackets as
   ...
   and owners tokens, to allow dots within either name itself.
