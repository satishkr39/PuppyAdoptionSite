...
class OracleDialect(default.DefaultDialect):
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        cursor = connection.execute(
            sql.text(
                ...
                "WHERE table_name = :name AND owners = :schema_name"
            ),
            dict(
    ...
    def _resolve_synonym(
        ...
        """search for a local synonym matching the given desired owners/name.

        if desired_owner is None, attempts to locate a distinct owners.

        returns the actual name, owners, dblink name, and synonym name if
        found.
        """

        q = (
            "SELECT owners, table_owner, table_name, db_link, "
        ...
        if desired_owner:
            clauses.append("owners = :desired_owner")
    ...
    def get_table_names(self, connection, schema=None, **kw):
        ...
        sql_str += (
            "OWNER = :owners " "AND IOT_NAME IS NULL " "AND DURATION IS NULL"
        )
    ...
    def get_view_names(self, connection, schema=None, **kw):
        schema = self.denormalize_name(schema or self.default_schema_name)
        s = sql.text("SELECT view_name FROM all_views WHERE owners = :owners")
        cursor = connection.execute(
    ...
    def get_table_options(self, connection, table_name, schema=None, **kw):
        ...
        if schema is not None:
            params["owners"] = schema
            text += " AND owners = :owners "
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        text = """
            ...
            AND col.owners = com.owners
            WHERE col.table_name = :table_name
            AND col.hidden_column = 'NO'
        """
        if schema is not None:
            params["owners"] = schema
            text += " AND col.owners = :owners "
    ...
    def get_table_comment(
        ...
        **kw
        ...
        COMMENT_SQL = """
            ...
            WHERE table_name = :table_name AND owners = :schema_name
        """
    ...
    def _get_constraint_data(
        ...
        if schema is not None:
            params["owners"] = schema
            text += "\nAND ac.owners = :owners"

        text += (
            "\nAND ac.owners = loc.owners"
            "\nAND ac.constraint_name = loc.constraint_name"
            "\nAND ac.r_owner = rem.owners(+)"
