...
class ResourceOwnerPasswordCredentialsGrant(GrantTypeBase):
    ...
    The resource owners password credentials grant type is suitable in
    cases where the resource owners has a trust relationship with the
    ...
    resource owners's credentials (username and password, typically using
    ...
    (A)  The resource owners provides the client with its username and
            password.

    (B)  The client requests an access token from the authorization
            server's token endpoint by including the credentials received
            from the resource owners.  When making the request, the client
            authenticates with the authorization server.

    (C)  The authorization server authenticates the client and validates
            the resource owners credentials, and if valid, issues an access
            token.
    ...
    def validate_token_request(self, request):
        ...
        username
                REQUIRED.  The resource owners username.

        password
                REQUIRED.  The resource owners password.
        ...
        o  validate the resource owners password credentials using its
            existing password validation algorithm.

        Since this access token request utilizes the resource owners's
